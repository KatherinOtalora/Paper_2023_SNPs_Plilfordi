...
setwd("/Users/katherin_otalora/Documents/GEA/GEA_intsamples_outliers_09_06_23")

#Read VCF and transform it to a genotype matrix
vcf <- read.vcfR("outliers_filtred_LOGPO_FST_comunes_bayescan_pcadapt_intsamples_withoutLD_1_100_K=8.recode.vcf", verbose = FALSE )
...
env  <- read.csv (file='Prueba_variables_ambientales_4.csv',sep=";",)
...
#Superficie y altura están correlacionadas a 0,87, y Capacidad biotica y altura 0.74.
...
#Nuestro R^2 dio 0.25 es decir un 25% de la variación. Este es un bajo poder explicativo de la variación y lo esperabamos asi, ya que nuestros datos son outliers y deberían mostrar una relación con los predicadores ambientales.
...
#Df Variance      F   Pr(>F)    
#Model     2   58.886 17.509 0.000999 ***
#  Residual 97  163.114                    
#---
...
#Model: rda(formula = gen.imp ~ Surface + Biotic_capacity, data = pred_1, scale = T)
#Df Variance       F   Pr(>F)    
#RDA1      1   58.267 34.6501 0.000999 ***
 # RDA2      1    0.618  0.3678 0.592408    
#Residual 97  163.114                     
#---
 # Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1


#Encontramos que el primer ejes restringido es significativos (p = 0.000999); el eje restringido 2 tiene un valor p de 0,8931, mientras que los eje 3 tienen valores p =0.8741. Esto se corresponde con nuestra evaluación del diagrama de escala, más arriba.
...
levels(env$Population) <- c("AIRE","ESCURT","ADRG","MOLTONA","GUARDIA","COLOM", "PRCV","RAVL")
eco <- env$Population

#Seleccionamos una paleta de colores para cada islote

bg <- c("#ff7f00","#1f78b4","#ffff33","#a6cee3","#1adce3","#e31a1c","#cce31a","#781ae3") # 8 nice colors for our ecotypes
...
write.table(rda_indv, "rda_plilfordi_100ind_intsamples_outliers_2_variables_4.txt", quote=FALSE, row.names=TRUE)
...
cand1 <- outliers(load.rda[,1],0.5) # 32
length(cand1)
cand2 <- outliers(load.rda[,2],0.5) #49
length(cand2)
cand3 <- outliers(load.rda[,3],0.5) #33
length(cand3)
...
write.table(cand, "snps_outliers_plilfordi_intsamples_2_variables_4.txt", quote=FALSE, row.names=TRUE)
...
write.table(cand, "correlation_snps_outliers_plilfordi_intsamples_2_variables_4.txt", quote=FALSE, row.names=TRUE)
...
table(foo[foo[,1]==2,2]) #  9 duplicates on axis 2

#0  1 
#20 29

table(foo[foo[,1]==3,2]) # 8 duplicates on axis 3
...
write.table(cand, "NEW_correlation_snps_outliers_plilfordi_intsamples_2_variables_4.txt", quote=FALSE, row.names=TRUE)


#We’ve now reduced our candidates to 52 unique SNPs.
...
write.table(cand, "predictor_ambiental_snps_outliers_plilfordi_intsamples_2_variables_4.txt", quote=FALSE, row.names=TRUE)
