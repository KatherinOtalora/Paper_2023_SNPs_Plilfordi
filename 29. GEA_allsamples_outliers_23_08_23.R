...
#env$Biotic_capacity <- log(env$Biotic_capacity) 
...
pred_1 <- subset(env, select=-c(Biotic_capacity))
str(pred_1)
pred_1 <- pred_1[,3:4]
str(pred_1)
pairs.panels(pred_1, scale=T)

#Nos quedamos solo con superficie y capacidad biotica con 0.94 de correlacion lo mas cercano a la regla empirica |r| > 0,7
...
#Model: rda(formula = gen.imp ~ Surface + Biotic_capacity, data = pred_1, scale = T)
#Df Variance       F   Pr(>F)    
#RDA1       1   2.3973 29.1652 0.000999 ***
 # RDA2       1   0.1494  1.8175 0.147852    
#Residual 188  15.4533                     
#---
 # Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
colnames(foo) <- c("Surface", "Maximum_Altitude")
